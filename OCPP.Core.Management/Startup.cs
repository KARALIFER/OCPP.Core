...
namespace OCPP.Core.Management
{
    public class Startup
    {
        ...
        private void EnsureDefaultUsers(IApplicationBuilder app)
        {
            ...
            {
                if (isSqlite && ex.Message.Contains("Users.UserId", StringComparison.OrdinalIgnoreCase))
                {
                    logger.LogWarning(ex, "SQLite user seeding failed with identity error. Retrying with explicit IDs.");
